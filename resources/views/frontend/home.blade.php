@extends('layouts.app')
@section('title',app()->getLocale()==='ar'?'ستايل فيرا — البسي قصتك':'Stylevera — Dress Your Story')
@section('content')
...
<section id="billboard" class="bg-light py-5">
  <div class="container">
    <div class="row justify-content-center">
      <h1 class="section-title text-center mt-4" data-aos="fade-up">{{ $locale==='ar'?'مجموعات جديدة':'New Collections' }}</h1>
      <div class="col-md-6 text-center" data-aos="fade-up" data-aos-delay="300">
        <p>{{ $locale==='ar'?'اكتشف أحدث صيحات الموضة الفاخرة في ستايل فيرا — وجهتك الأولى للأزياء في الإمارات.':'Discover the latest luxury fashion at Stylevera — your premier fashion destination in the UAE.' }}</p>
      </div>
    </div>
    <div class="row">
          ...
          <div class="swiper-slide">
            <div class="banner-item image-zoom-effect">
              <div class="image-holder">
                <a href="{{ $banner->link ?: route('shop') }}">
                  @if($banner->image && str_starts_with($banner->image,'http'))
                    <img src="{{ $banner->image }}" alt="{{ $banner->titleLocale() }}" class="img-fluid">
                  @else
                    <img src="{{ asset('images/placeholder.svg') }}" alt="{{ $banner->titleLocale() }}" class="img-fluid" style="min-height:400px;object-fit:cover;background:#f1f1f0">
                  @endif
                </a>
              </div>
              <div class="banner-content py-4">
                <h5 class="element-title text-uppercase"><a href="{{ $banner->link ?: route('shop') }}" class="item-anchor">{{ $banner->titleLocale() }}</a></h5>
                <p>{{ $banner->subtitleLocale() }}</p>
                <div class="btn-left"><a href="{{ $banner->link ?: route('shop') }}" class="btn-link fs-6 text-uppercase text-decoration-none">{{ $banner->btnLocale() }}</a></div>
              </div>
            </div>
          </div>
          @empty
...
<section class="features py-5">
  <div class="container">
    <div class="row">
      @foreach([['calendar','Book An Appointment','احجز موعداً'],['shopping-bag','Pick Up In Store','استلم من المتجر'],['gift','Special Packaging','تغليف مميز'],['arrow-cycle','Free Global Returns','إرجاع مجاني دولي']] as $i=>[$icon,$en,$ar])
      <div class="col-md-3 text-center" data-aos="fade-in" data-aos-delay="{{ $i*200 }}">
        <div class="py-5">
          <svg width="38" height="38" viewBox="0 0 24 24"><use xlink:href="#{{ $icon }}"/></svg>
          <h4 class="element-title text-capitalize my-3">{{ $locale==='ar'?$ar:$en }}</h4>
          <p>{{ $locale==='ar'?'نقدم لك أفضل تجربة تسوق في الإمارات.':'We bring you the best shopping experience in the UAE.' }}</p>
        </div>
      </div>
      @endforeach
    </div>
...
<section class="categories overflow-hidden">
  <div class="container">
    <div class="open-up" data-aos="zoom-out">
      <div class="row">
        @forelse($featuredCats as $i=>$cat)
        <div class="col-md-4">
          <div class="cat-item image-zoom-effect">
            <div class="image-holder">
              <a href="{{ route('shop',['cat'=>$cat->slug]) }}">
                @if($cat->image && str_starts_with($cat->image,'http'))
                  <img src="{{ $cat->image }}" alt="{{ $cat->nameLocale() }}" class="product-image img-fluid">
                @else
                  <img src="{{ asset('images/placeholder.svg') }}" alt="{{ $cat->nameLocale() }}" class="product-image img-fluid" style="min-height:300px;object-fit:cover">
                @endif
              </a>
            </div>
            <div class="category-content"><div class="product-button">
              <a href="{{ route('shop',['cat'=>$cat->slug]) }}" class="btn btn-common text-uppercase">{{ $locale==='ar'?'تسوق '.$cat->nameLocale():'Shop '.$cat->nameLocale() }}</a>
            </div></div>
          </div>
        </div>
        @empty
...
<section id="new-arrival" class="new-arrival product-carousel py-5 position-relative overflow-hidden">
  <div class="container">
    <div class="d-flex flex-wrap justify-content-between align-items-center mt-5 mb-3">
      <h4 class="text-uppercase" style="letter-spacing:1.5px">{{ $locale==='ar'?'أحدث المنتجات':'Our New Arrivals' }}</h4>
      <a href="{{ route('shop',['sort'=>'newest']) }}" class="btn-link">{{ $locale==='ar'?'عرض الكل':'View All Products' }}</a>
    </div>
          ...
          <div class="product-item image-zoom-effect link-effect">
            <div class="image-holder position-relative">
              ...
              <a href="{{ route('product.show',$p->slug) }}">
                <video src="{{ $primary->url }}" muted autoplay loop class="product-image" style="width:100%;object-fit:cover;aspect-ratio:3/4"></video>
              </a>
              @elseif($primary)
              <a href="{{ route('product.show',$p->slug) }}"
                @if($imgUrls->count()>1)
                  class="sv-multi-img"
                  data-original-src="{{ $primary->url }}"
                  data-images="{{ $imgUrls->join('||') }}"
                @endif>
              ...
              @else
              <a href="{{ route('product.show',$p->slug) }}">
                <img src="{{ asset('images/placeholder.svg') }}" alt="{{ $p->nameLocale() }}" class="product-image img-fluid" style="aspect-ratio:3/4;object-fit:cover">
              </a>
              @endif
              ...
              <div class="product-content">
                ...
                <a href="{{ route('product.show',$p->slug) }}" class="text-decoration-none" data-after="{{ $locale==='ar'?'أضف للسلة':'Add to cart' }}">
                  <span class="price-aed">
                    @if($dp['sale'])<span class="original">{{ number_format($dp['price'],2) }} AED</span><span class="sale">{{ number_format($dp['sale'],2) }} AED</span>
                    @else{{ number_format($dp['price'],2) }} AED @endif
                  </span>
                </a>
              </div>
            </div>
      ...
      <div class="collection-item d-flex flex-wrap my-5">
        ...
        <div class="col-md-6 column-container bg-white">
          <div class="collection-content p-5 m-0 m-md-5">
            <h3 class="element-title text-uppercase">{{ $locale==='ar'?'مجموعة الشتاء الكلاسيكية':'Classic Winter Collection' }}</h3>
            <p>{{ $locale==='ar'?'اكتشف مجموعتنا الشتوية الفاخرة — مصممة خصيصاً لتناسب أسلوب حياتك في دولة الإمارات.':'Discover our luxurious winter collection — designed for the UAE lifestyle with premium fabrics and timeless cuts.' }}</p>
            <a href="{{ route('shop') }}" class="btn btn-dark text-uppercase mt-3" style="letter-spacing:2px">{{ $locale==='ar'?'تسوق المجموعة':'Shop Collection' }}</a>
          </div>
        </div>
      </div>
    </div>
...
<section id="best-sellers" class="best-sellers product-carousel py-5 position-relative overflow-hidden">
  <div class="container">
    <div class="d-flex flex-wrap justify-content-between align-items-center mt-5 mb-3">
      <h4 class="text-uppercase" style="letter-spacing:1.5px">{{ $locale==='ar'?'الأكثر مبيعاً':'Best Selling Items' }}</h4>
      <a href="{{ route('shop') }}" class="btn-link">{{ $locale==='ar'?'عرض الكل':'View All Products' }}</a>
    </div>
    <div class="swiper product-swiper open-up" data-aos="zoom-out">
      <div class="swiper-wrapper d-flex">
        ...
        <div class="swiper-slide">
          <div class="product-item image-zoom-effect link-effect">
            <div class="image-holder position-relative">
              @if($primary && $primary->type==='video')
              <a href="{{ route('product.show',$p->slug) }}"><video src="{{ $primary->url }}" muted autoplay loop class="product-image" style="width:100%;object-fit:cover;aspect-ratio:3/4"></video></a>
              @elseif($primary)
              <a href="{{ route('product.show',$p->slug) }}"
                @if($imgUrls->count()>1) class="sv-multi-img" data-original-src="{{ $primary->url }}" data-images="{{ $imgUrls->join('||') }}" @endif>
                <img src="{{ $primary->url }}" alt="{{ $p->nameLocale() }}" class="product-image sv-main-img img-fluid" onerror="this.src='{{ asset('images/placeholder.svg') }}'">
              </a>
              @if($imgUrls->count()>1)<div class="sv-img-dots">@for($d=0;$d<min($imgUrls->count(),5);$d++)<div class="sv-img-dot {{ $d===0?'active':'' }}"></div>@endfor</div>@endif
              @else
              <a href="{{ route('product.show',$p->slug) }}"><img src="{{ asset('images/placeholder.svg') }}" alt="{{ $p->nameLocale() }}" class="product-image img-fluid" style="aspect-ratio:3/4;object-fit:cover"></a>
              @endif
              <a href="#" class="btn-icon btn-wishlist" onclick="svToggleWishlist({{ $p->id }},this);return false"><svg width="24" height="24" viewBox="0 0 24 24"><use xlink:href="#heart"/></svg></a>
              <div class="product-content">
                <h5 class="text-uppercase fs-5 mt-3"><a href="{{ route('product.show',$p->slug) }}">{{ $p->nameLocale() }}</a></h5>
                <a href="{{ route('product.show',$p->slug) }}" class="text-decoration-none" data-after="{{ $locale==='ar'?'أضف للسلة':'Add to cart' }}">
                  <span class="price-aed">{{ number_format($dp['sale']??$dp['price'],2) }} AED</span>
                </a>
              </div>
            </div>
          </div>
...
<section class="testimonials py-5 bg-light">
  <div class="section-header text-center mt-5">
    <h3 class="section-title" style="letter-spacing:2px">{{ $locale==='ar'?'ماذا يقول عملاؤنا':'WE LOVE GOOD COMPLIMENT' }}</h3>
  </div>
  <div class="swiper testimonial-swiper overflow-hidden my-5">
    <div class="swiper-wrapper d-flex">
      @foreach([['More than expected — crazy soft, flexible, and perfectly fitted.','casual way'],['"Best fitted shirt — more than expected, crazy soft and flexible."','uptop'],['Outstanding quality and elegant design — Stylevera is truly the best!','Denim craze']] as [$q,$by])
      <div class="swiper-slide">
        <div class="testimonial-item text-center">
          <blockquote><p style="font-style:italic">"{{ $q }}"</p><div class="review-title text-uppercase" style="font-size:12px;letter-spacing:2px;color:var(--sv-primary)">— {{ $by }}</div></blockquote>
        </div>
      </div>
      @endforeach
    </div>
