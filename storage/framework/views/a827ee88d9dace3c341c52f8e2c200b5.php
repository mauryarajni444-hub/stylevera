...
<div class="row g-4">
  <div class="col-lg-5">
    <div class="sv-card">
      ...
      <form method="POST" action="<?php echo e(route('admin.banners.store')); ?>"><?php echo csrf_field(); ?>
        <div class="row g-3">
  ...
  <div class="col-lg-7"><div class="sv-card"><table class="sv-table">
    ...
    <tbody><?php $__currentLoopData = $banners; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $b): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <tr>
      <td><img src="<?php echo e($b->image); ?>" style="width:60px;height:40px;object-fit:cover"></td>
      <td><?php echo e($b->title_en); ?></td>
      <td><?php echo e($b->position); ?></td>
      <td><span class="sv-badge <?php echo e($b->is_active?'sv-b-active':'sv-b-inactive'); ?>"><?php echo e($b->is_active?'Active':'Off'); ?></span></td>
      <td>
        <div class="upload-zone" style="padding:6px;font-size:11px" onclick="document.getElementById('svBI_<?php echo e($b->id); ?>').click()">Upload New
          <input type="file" id="svBI_<?php echo e($b->id); ?>" accept="image/*" style="display:none" onchange="uploadBannerImg(this.files,<?php echo e($b->id); ?>)">
        </div>
        <form method="POST" action="<?php echo e(route('admin.banners.destroy',$b->id)); ?>" onsubmit="return confirm('Delete?')" style="margin-top:4px"><?php echo csrf_field(); ?><button class="btn btn-sm btn-outline-danger w-100" style="font-size:11px">Del</button></form>
      </td>
    </tr><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></tbody>
  </table><div class="mt-3"><?php echo e($banners->links()); ?></div></div></div>
</div>
...
function uploadBannerImg(files, bid) {
  ...
  const fd = new FormData(); fd.append('file', files[0]); fd.append('_token', '<?php echo e(csrf_token()); ?>');
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH F:\stylevera_project\resources\views/admin/banners/index.blade.php ENDPATH**/ ?>